...
async def wait_for_captured_keys(captured, keys, timeout_ms, interval_ms=80):
    deadline = asyncio.get_running_loop().time() + (timeout_ms / 1000)
    while asyncio.get_running_loop().time() < deadline:
        if all(captured.get(key) is not None for key in keys):
            return True
...
async def collect_recent_match_urls(page, team_url, max_matches=10):
    ...
    """
    await page.goto(team_url, wait_until="domcontentloaded", timeout=NAVIGATION_TIMEOUT_MS)
